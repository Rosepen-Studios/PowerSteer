...
print("Trackmania server config launched")
...
def compute_z_axis(brake):
    return 32768 if brake>0 else 0

def compute_y_axis(accelerate):
    return 32768 if accelerate>0 else 0
...
while True:
    ...
    try:
        ...
        y_val = compute_y_axis(accelerate)
        z_val = compute_z_axis(brake)

        j.set_axis(pyvjoy.HID_USAGE_X, x_val)
        j.set_axis(pyvjoy.HID_USAGE_Y, y_val)
        j.set_axis(pyvjoy.HID_USAGE_Z, z_val)

        print(f"ROLL: {roll}, ACCEL: {accelerate}, BRAKE: {brake} => X: {x_val}, Y: {y_val}, Z: {z_val}")
